import { SkRes } from '../../base/models/response'
import GameController from '../../base/controller'
import GamePlay from '../../base/models/game-play'
import Room from '../../base/models/room'
import User from '../../base/models/user'
import GalaxyGame from '../models/galaxy-game'
import GalaxyRoom from '../models/galaxy-room'
import { Direction, SkCode } from '../../base/models/response'
import { NextEmit } from '../../base/models/typing'
...
export default class GalaxyController extends GameController {
    ...
    startGamesInRoom(ownerId: string, next: NextEmit<GalaxyRoom>) {
        let r = this.getRoom(ownerId)
        if (!r) {
            r = new GalaxyRoom(ownerId)
            r.addUser(ownerId, this.getUser(ownerId))
            this.addRoom(r)
        } else {
            r.games = {}
            r.addGame(ownerId, new GalaxyGame(ownerId))
        }
